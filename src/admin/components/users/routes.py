...
def get_all_users():
    if check_current_user():
        ...
        return render_template(
            "admin/users/all-users.html",
            users=paginated_users,
            title="Все пользователи",
            pagination=get_pagination(page=page, per_page=PER_PAGE, total=users),
        )
    return redirect("/")
...
def get_all_staff_users():
    if check_current_user():
        ...
        paginated_users = get_paginated_staff(page=page, staff=users, per_page=PER_PAGE)
        return render_template(
            "admin/users/all-staff-users.html",
            users=paginated_users,
            title="Весь персонал",
            pagination=get_pagination(page=page, per_page=PER_PAGE, total=users),
        )
...
def get_user_profile(user_id: int):
    if check_current_user():
        if request.method == "POST":
            address_conf = check_address_conf(
                request.form.get("address_conf_1"), request.form.get("address_conf_2")
            )
            email_conf = check_email_conf(request.form.get("email_conf"))
            phone_cong = check_phone_conf(request.form.get("phone_conf"))
            is_staff, is_superuser = check_is_staff(
                request.form.get("is_staff"), request.form.get("is_superuser")
            )

            detail, status = AdminUsersQueries.update_user(
        ...
        return render_template(
            "admin/users/user-profile.html", user=user, title="Профиль пользователя"
        )
    return redirect("/")
